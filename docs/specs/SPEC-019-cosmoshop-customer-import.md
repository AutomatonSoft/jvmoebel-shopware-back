...
влияют на выбор default shipping. У 237 address-строк country подтверждена;
ещё три строки содержат неизвестные source country ID `7`, `18` и `45` и
должны попасть в ошибки, а не получить угаданную страну. У всех 240 строк
заполнены обязательные first name, last name, street и city; превышений
Shopware max length в проверенном snapshot нет.
...
Внутри статуса `a` 818 записей имеют подтверждённую дату и переходят в
`optIn`, ещё 728 не имеют её и переходят в `direct`.
