...
final readonly class PrepareCatalogShopwareProductImportRecordService
{
    ...
    public function execute(array $row, Context $context): array
    {
        ...
        $schemas = [];
        foreach ($this->schemaProvider->forSource('okb', [$groupId], $context) as $schema) {
    ...
    {
        $translations = [];
        foreach ($this->connection->fetchFirstColumn('SELECT LOWER(HEX(`id`)) FROM `language`') as $languageId) {
