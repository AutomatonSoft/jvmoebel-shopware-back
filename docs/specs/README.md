...
| [SPEC-054-product-option-templates](SPEC-054-product-option-templates.md) | Шаблоны опций товара (плагин `JvProductOptions`): назначение через product streams или вручную, надбавки `fixed`/`percentage`, Store API опций, расчёт корзины и снимок в заказе |
