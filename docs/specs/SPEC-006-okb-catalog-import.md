...
детерминированным ID. Для каждой `category_id` создаётся её дочерняя category
 с детерминированным ID. Обновление сохраняет ID и не создаёт копий. Параметр
 `--group-parent-id=<Shopware category UUID>` помещает все OKB groups под
 указанный узел; без него parent не изменяется.

Два будущих пользовательских уровня создаются отдельно. После их создания
группы обновляются только через `parentId` (в том числе повторным вызовом
структурной команды с `--group-parent-id`); OKB groups и их children не
