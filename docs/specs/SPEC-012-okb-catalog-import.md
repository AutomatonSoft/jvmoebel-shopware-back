# SPEC-012 — Импорт каталожной схемы OKB
...
Каждая строка входного CosmoShop CSV находит одну OKB variation по своему EAN,
а через её `productReference` — всё семейство вариантов этого товара. Запрос
семейства передаёт явный `limit`: без него OKB отдаёт только первую страницу из
десяти вариаций и молча отбрасывает остальные. Если очередная страница
повторяет предыдущую, пагинация не продвигается: поиск семейства завершается
ошибкой, и товар попадает в failures вместо бесконечного перебора страниц.

Семейство состоит только из вариаций с тем же `productReference` и той же
категорией, что у вариации исходного товара; остальные записи ответа в Shopware
не попадают. Вариация исходного товара входит в семейство всегда, даже если
ответ на запрос семейства её не содержит.

Исходный CosmoShop product становится Shopware parent с прежним `productNumber`
и сохраняет свою цену и свой EAN: это базовая карточка, а не цена варианта.
EAN parent остаётся EAN исходного товара и после обогащения; он совпадает с EAN
варианта в позиции `1`, и это допустимо.

Child идентифицируется парой parent и EAN варианта. Его Shopware ID строится из
этой пары, а `productNumber` имеет вид `<parent productNumber>-<номер>`. Номер
присваивается при первом появлении варианта и после этого не меняется, даже если
в семействе появились новые варианты или сменился их порядок: корзины, списки
желаний и заказы продолжают указывать на ту же вариацию. Новый вариант получает
следующий свободный номер этого parent. При первом импорте семейства номер `1`
получает вариант, чей EAN совпадает с EAN исходного товара, остальные следуют за
ним по возрастанию EAN. Номер определяется по уже записанным child, а не по
порядку обработки строк, поэтому он не зависит от того, на какие пакеты импорт
разбил файл, и от того, какие строки были отклонены.

Вариантом товара в Shopware является сочетание значений тех атрибутов, которые
снимок помечает как `VARIATION_THEME`. Если у нескольких вариаций семейства это
сочетание совпадает, они попадают в Shopware одним child: OKB содержит
задвоенные карточки, отличающиеся только EAN. Сохраняется вариация с EAN
исходного товара, иначе наименьшая по EAN. Значения одного атрибута сравниваются
как набор: их порядок не важен, а значение, содержащее разделитель, не
совпадает с несколькими отдельными значениями. Остальные поля вариации на это
сравнение не влияют.

`productReference` используется только для получения семейства. Разные CosmoShop
products не объединяются автоматически, даже если их семейства пересекаются.

Правило для исчезнувшего варианта этой итерацией не определяется: если при
повторном прогоне OKB больше не возвращает ранее импортированный вариант, его
child остаётся в Shopware без изменений.
...
рынком; несовпадающая или отсутствующая цена является ошибкой строки. Цена OKB
должна быть положительной: отрицательная или нулевая отклоняет variation, а не
заменяется ценой parent. Десятичная запятая допускается и читается так же, как
точка, одним и тем же значением при проверке и при записи.

OKB `msrp` используется как рекомендованная цена только в той же валюте, что и
`standardPrice`; `msrp` в другой валюте не учитывается.

Повторный импорт обновляет у child цену только в валюте рынка. Цены child в
других валютах сохраняются; цены parent служат лишь начальными значениями для
нового child.
...
Импорт идёт пакетами ядра, и строки одного семейства могут оказаться в разных
пакетах, поэтому семейство проверяется по строкам, а не целиком. Проверка строки
parent не зависит от attributes и цены какой-либо variation: parent получает
категорию, даже если исходная variation отклонена. Каждая variation проверяется
отдельно; отклонённая попадает в invalid-records, остальные variations того же
семейства записываются. `Markeninformationen` не импортируется:
это маркетинговый текст о собственном бренде, а каталог CosmoShop практически
целиком принадлежит одному производителю. Описание производителя ведётся
вручную и импортом не изменяется. При
