...
final class OkbCsvReaderTest extends TestCase
{
    public function testItReadsBomDelimitedRows(): void
    {
        ...
        try {
            $rows = iterator_to_array((new OkbCsvReader())->rows($file, ['category_id', 'name']));
    ...
    public function testItRejectsRowsWithADifferentColumnCount(): void
    {
        ...
        try {
            ...
            iterator_to_array((new OkbCsvReader())->rows($file, ['category_id', 'name']));
        } finally {
            unlink($file);
        }
    }
    ...
    {
        $file = tempnam(sys_get_temp_dir(), 'jv-okb-csv-');
