...
/**
 * Resolves CMS element `jv-expert-profile` for the Store API (platform SPEC-026 / backend SPEC-034).
 */
final class ExpertProfileCmsElementResolver extends AbstractCmsElementResolver
{
    public const string TYPE = 'jv-expert-profile';
    ...
    public function enrich(CmsSlotEntity $slot, ResolverContext $resolverContext, ElementDataCollection $result): void
    {
        ...
        $slot->setData(new ExpertProfileStruct(
            name: $this->requiredString($config->get('name')?->getValue()),
            role: $this->optionalString($config->get('role')?->getValue()),
    ...
    private function normalizeLink(mixed $value): ?ExpertProfileLinkStruct
    {
        if (!\is_array($value)) {
            return null;
        }
        ...
        return new ExpertProfileLinkStruct($label, $url);
    }

    private function resolveImage(?MediaEntity $entity): ?ExpertProfileMediaStruct
    {
        if (null === $entity) {
            return null;
        }
        ...
        return new ExpertProfileMediaStruct($url, $alt);
    }
    ...
    {
        return 'jv_expert_profile_media_'.$slot->getUniqueIdentifier();
    }
}
