...
/**
 * Resolves CMS element `jv-guide-hub-cards` for the Store API (platform SPEC-028 / backend SPEC-036).
 */
final class GuideHubCardsCmsElementResolver extends AbstractCmsElementResolver
{
    public const string TYPE = 'jv-guide-hub-cards';
     ...
     */
    private function normalizeCards(mixed $value, array $media): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            $normalized[] = new GuideHubCardStruct(
                id: $id,
    ...
    private function resolveImage(?MediaEntity $entity): ?GuideHubCardMediaStruct
    {
        if (null === $entity) {
            return null;
        }
        ...
        return new GuideHubCardMediaStruct($url, $alt);
    }
    ...
    {
        return 'jv_guide_hub_cards_media_'.$slot->getUniqueIdentifier();
    }
}
