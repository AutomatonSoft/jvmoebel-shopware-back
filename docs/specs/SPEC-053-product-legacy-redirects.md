...
Корневая запись редиректа хранит флаг import_locked, который по умолчанию
отключён. Менять его можно в ручной форме создания или редактирования редиректа
в Administration JvSeo. Флага нет в DTO продуктового и image imports.

Когда флаг включён, batch-импорты JvSeo пропускают весь агрегат с результатом
manualPreserved: не добавляют и не обновляют его каналы и исходные URL. Блокировка
действует на все Sales Channels агрегата.

Отключение флага не снимает существующие отметки ручного изменения у отдельных
каналов и исходных URL. Для них продолжает действовать текущее поведение ручного
редактирования.

Интеграционные тесты проверяют выключенное значение по умолчанию, ручное
включение и пропуск product и image imports для заблокированных агрегатов.
