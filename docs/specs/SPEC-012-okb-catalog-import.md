...
десяти вариаций и молча отбрасывает остальные.
...
варианта в позиции `1`, и это допустимо. Каждый вариант
семейства становится child с `productNumber` `<parent productNumber>-<позиция>`.
Позицию `1` получает вариант, чей EAN совпадает с EAN исходного товара, остальные
следуют за ним по возрастанию EAN. Нумерация поэтому не зависит от порядка, в
котором OKB вернул семейство, и не меняется между прогонами.
...
исходного товара, иначе наименьшая по EAN. Остальные поля вариации на это
сравнение не влияют.
...
Все строки товара повторяют входные attributes, поэтому ошибка валидации
останавливает и parent, и всех его child. `Markeninformationen` не импортируется:
