...
- сохранение основной фабрики товара в локальном каталоге по SPEC-055 после
...
- постановка OKB-обогащения по SPEC-012 после завершения прогона и активация
  родителя, для которого OKB вернул семейство вариантов.
...
- публикация нового товара без категории: новый товар создаётся неактивным;
  немецкая visibility существует, но товар не публикуется, пока OKB-обогащение
  не найдёт для него семейство либо администратор не включит его вручную;
- деактивация товара, для которого OKB ничего не вернул: такой товар и так
  остаётся неактивным, отдельного правила для него нет.
...
Нормализатор принимает только каноническую положительную десятичную строку,
без leading zero и переполнения PHP int; дробное число, знак, zero и boolean
не принимаются. Только составной lock key
...
## Обогащение OKB после импорта

Как только прогон переходит в terminal state `completed` или
`completed_with_errors`, для него ставится ровно одно сообщение OKB-обогащения.
Сообщение отправляется после фиксации checkpoint, а не внутри его транзакции:
...
Дальше работает тот же конвейер, что и после импорта CosmoShop. Отличаются
только источник строк и ключ, по которому обогащение защищено от повтора: для
CosmoShop это ID Import/Export лога, для Aftercool — ID прогона. Повторная
доставка сообщения находит уже созданный каталожный лог по этому ключу и не
создаёт второй.

Если OKB вернул семейство, родитель становится активным. Это единственное
место, где Aftercool-процесс включает товар. Дети наследуют `active` от
родителя и собственного значения не получают.

Товар может одновременно принадлежать и Aftercool, и CosmoShop: оба процесса
строят UUID и `productNumber` из EAN. Для такого товара следующий импорт
CosmoShop снова запишет `active` из своего `sourceInactive` и перекроет
активацию. Выигрывает последний записавший; это ожидаемое поведение.
...
- три исходные DAL entities и migrations для run, source link и report entry;
  каталог фабрик и связь товара добавляются отдельно по SPEC-055;
...
перенос существующих source links определены [SPEC-055](SPEC-055-product-factories.md).
