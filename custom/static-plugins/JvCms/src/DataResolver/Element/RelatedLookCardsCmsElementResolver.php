...
/**
 * Resolves CMS element `jv-related-look-cards` for the Store API (platform SPEC-017 / backend SPEC-025).
 *
 ...
 */
final class RelatedLookCardsCmsElementResolver extends AbstractCmsElementResolver
{
    public const string TYPE = 'jv-related-look-cards';
    ...
    public function enrich(CmsSlotEntity $slot, ResolverContext $resolverContext, ElementDataCollection $result): void
    {
        ...
        $mediaMap = $this->mediaMap($result->get('jv_related_look_cards_media_'.$slot->getUniqueIdentifier()));

        $slot->setData(new RelatedLookCardsStruct(
    ...
    private function normalizeCards(mixed $value, array $mediaMap): array
    {
        ...
        foreach ($entries as $entry) {
            ...
            $normalized[] = new RelatedLookCardStruct(
                id: $id,
    ...
    private function resolveImage(?MediaEntity $entity): ?RelatedLookCardMediaStruct
    {
        if (null === $entity) {
            return null;
        }
        ...
        return new RelatedLookCardMediaStruct($url, $alt);
    }
