...
final readonly class ImportImageRedirectsService implements ImportImageRedirectsInterface
{
    /**
    ...
    public function import(iterable $redirects, Context $context): ImageRedirectImportResult
    {
        ...
        return new ImageRedirectImportResult($created, $updated, $unchanged, $manualPreserved, $conflicts, $invalid, $issues);
    }

    private function importOne(ImportImageRedirectData $data, Context $context): string
    {
        ...
        if (!Uuid::isValid($data->mediaId) || !Uuid::isValid($data->salesChannelId)) {
            throw new \InvalidArgumentException('Media and Sales Channel IDs must be valid UUIDs.');
        }
        ...
        if ($byImportKey instanceof RedirectSourceEntity) {
            ...
            if (!$existingTarget instanceof RedirectEntity || $existingTarget->getMediaId() !== $data->mediaId || $existingChannel->getSalesChannelId() !== $data->salesChannelId) {
                return 'conflict';
            }
        ...
        if ($collision instanceof RedirectSourceEntity && $collision->getId() !== $byImportKey?->getId()) {
            ...
            if ($collisionTarget instanceof RedirectEntity
                && $collisionTarget->getMediaId() === $data->mediaId
    ...
    private function findImageRedirect(string $mediaId, Context $context): ?RedirectEntity
    {
        $criteria = (new Criteria())
            ->addFilter(new EqualsFilter('type', RedirectType::Image->value))
    ...
    private function issue(ImportImageRedirectData $data, string $code, string $message): array
    {
        return [
